    ...
    , ui(new Ui::Abonnes)
{
  ...
  ui->li_id->setValidator(new QIntValidator(00000000, 99999999, this));
  ui->li_id_e->setValidator(new QIntValidator(0, 99999999, this));
  ui->li_salle->setValidator(new QIntValidator(00000000, 99999999, this));
  ui->li_cap->setValidator(new QIntValidator(0, 99999999, this));
  ui->show_abonnes->setModel(S.afficher());
   ...
   animation= new QPropertyAnimation(ui->pb_Abonne,"geometry");
       ...
       animation->setEndValue(QRect(860,500,91,41));
       animation->start();
       animation= new QPropertyAnimation(ui->modifier_Abonnes,"geometry");
       animation->setDuration(2500);
       animation->setEndValue(QRect(960,500,91,41));
       animation->start();
       animation= new QPropertyAnimation(ui->supprimer_Abonnes,"geometry");
       animation->setDuration(1500);
       animation->setEndValue(QRect(910,560,91,41));
       animation->start();

       QMediaPlayer * music =new QMediaPlayer;
            music->setMedia(QUrl("C://Users/21650/Desktop/Gestion_abonnes/welcome.mp3")) ;
...
if(S.verifierCin_aj(S.getCin())==false)
   {
       change(ui->wrong_cin,"<font color='red'> Cet identifiant est déjà utilisé</font>",7000, 0);
   }
if(S.veriftel(S.getCin())==false)
   {
       change(ui->wrong_cin,"<font color='red'> CIN doit comporter exactement 8 chiffres</font>",7000, 0);
   }
if(S.veriftel(S.getNum_tel())==false)
   {
       change(ui->wrong_tel, "<font color='red'>Le numéro de téléphone doit comporter exactement 8 chiffres</font>",7000, 0);
   }
if(S.verifvidestring(S.getNom())==false)
   {
       change(ui->wrong_nom, "<font color='red'>Ce champs requis</font>",7000, 0);
   }
if(S.verifvidestring(S.getPrenom())==false)
   {
       change(ui->wrong_prenom, "<font color='red'>Ce champs requis</font>",7000, 0);
   }
if(S.verifvidestring(S.getEmail())==false)
   {
       change(ui->wrong_email, "<font color='red'>Ce champs requis</font>",7000, 0);
   }
if(S.verifvidestring(S.gettype_abonnement())==false)
   {
       change(ui->wrong_type, "<font color='red'>Ce champs requis</font>",7000, 0);
   }
if(S.verifvidestring(S.getsexe_abonnes())==false)
   {
       change(ui->wrong_sexe, "<font color='red'>Ce champs requis</font>",7000, 0);
   }
if( S.verifierCin_aj(S.getCin())==true && S.veriftel(S.getNum_tel())==true && S.verifvidestring(S.getNom())==true &&
            S.verifvidestring(S.getPrenom())==true && S.verifvidestring(S.getEmail())==true && S.verifvidestring(S.gettype_abonnement())==true &&
            S.verifvidestring(S.getsexe_abonnes())==true)
       {
bool test=S.ajouter();
        if(test)
            {
...
void Abonnes::on_pb_Seance_clicked()
{
    ...
    if(A.veriftel(A.getID_salle())==false)
       {
           change(ui->wrong_ids, "<font color='red'>Ce champs requis</font>",7000, 0);
       }
    if(A.verifid(A.getID_employe())==false)
       {
           change(ui->wrong_ide, "<font color='red'>CIN doit comporter exactement 8 chiffres</font>",7000, 0);
       }
    if(A.veriftel(A.getNum_salle())==false)
       {
           change(ui->wrong_salle, "<font color='red'>Ce champs requis</font>",7000, 0);
       }
    if(A.veriftel(A.getCapacite_salle())==false)
       {
           change(ui->wrong_cap, "<font color='red'>Ce champs requis</font>",7000, 0);
       }
    if(A.verifvidestring(A.getTitre_cours())==false)
       {
           change(ui->wrong_titre, "<font color='red'>Ce champs requis</font>",7000, 0);
       }
    if(A.verifvidestring(A.getNiveau_seance())==false)
       {
           change(ui->wrong_niv, "<font color='red'>Ce champs requis</font>",7000, 0);
       }
if(A.veriftel(A.getID_salle())==true && A.verifid(A.getID_salle())==true && A.veriftel(A.getNum_salle())==true && A.veriftel(A.getCapacite_salle())
       ==true && A.verifvidestring(A.getTitre_cours())==true && A.verifvidestring(A.getNiveau_seance())==true){
    ...
    QSqlQuery qry;
       ...
       if(qry.exec())
       {
           while(qry.next())
           {
               ...
               ui->li_tel->setText(qry.value(4).toString());
               ui->li_date_s->setDate(qry.value(5).toDate());
               ui->li_ab->setText(qry.value(6).toString());
